...
let lang = getParameterByName('lang').toUpperCase(); 
let title;
  ...
  localization["All Terms"].forEach((item,i)=>{
    if(item._CMSKEY==optName){
	  ...
	  innerdivNode.addEventListener("click",()=>{
		...
		const newLocation =
		"./"+lang.toLowerCase()+"/rules.html?v=" + Date.now()+'&page='+itemID+'&lang='+lang;
			window.location.href = newLocation;
			window.parent.window.postMessage({
				topic: "changeTitle",
				data: {title:title,pageName:item[lang]},
			  }, "*");
...
for (let i=0;i<localization["All Terms"].length;i++){
  ...
  innerdivNode.addEventListener("click",()=>{
	...
	const newLocation =
		...
		window.parent.window.postMessage({
			topic: "changeTitle",
			data: {title:title,pageName:localization["All Terms"][i][lang]},
		  }, "*");
