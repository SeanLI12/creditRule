  ...
  if (!event.target.matches(".dropbtn")) {
    if (event.target.matches(".menu-item")) {
      ...
      window.parent.parent.postMessage({
          topic: "changeTitle",
          data: {title:"",pageName:document.getElementById(itemID).innerText},
      }, "*");

      const newLocation =
        "./sports/" + event.target.id + ".html?v=" + Date.now();
  ...
  if (sportsViewer.src.includes("/sports/football.html")) {
    ...
    linkTable.addEventListener("click", function (event) {
      ...
      if (event.target.matches("a")) {
        ...
        var targetAnchorDom = sportsViewer.contentDocument.querySelector(
          'a[id="' + anchorName + '"]'
        );
